...
class FFModel(BaseModel):
    ...
    #############################
    def statistics_ff_dict(self, data_statistics):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        feed_d.update(self.statistics_ff_dict(data_statistics))
        return self.sess.run([self.next_obs_pred], feed_dict=feed_d)[0]

    def update(self, observations, actions, next_observations, data_statistics):
        # train the model
        feed_d = {self.obs_pl: observations, self.acs_pl: actions, self.delta_labels: next_observations-observations}
        feed_d.update(self.statistics_ff_dict(data_statistics))
        _, loss = self.sess.run([self.train_op, self.loss], feed_dict=feed_d)
        return loss